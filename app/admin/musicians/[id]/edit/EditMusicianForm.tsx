...
function getStatus(stav: Uint8Array): string {
...
export default function EditMusicianForm({
  musician,
  bands,
}: Props) {
  const [name, setName] =
    useState(musician.name);

  const [realName, setRealName] =
    useState(musician.real_name);

  const [instrument, setInstrument] =
    useState(musician.instrument);

  const [interpret, setInterpret] =
    useState(String(musician.interpret));

  const [birthYear, setBirthYear] =
    useState(
      getYear(musician.date_of_birth)
    );

  const [placeOfBirth, setPlaceOfBirth] =
    useState(
      musician.place_of_birth ?? ""
    );

  const [deathYear, setDeathYear] =
    useState(
      getYear(musician.date_of_dead)
    );

  const [pusobeni, setPusobeni] =
    useState(
      musician.pusobeni ?? ""
    );
  ...
  const [status, setStatus] =
    useState(
      getStatus(musician.stav)
    );

  const [text, setText] =
    useState(musician.text);
  ...
  async function handleSubmit(
    ...
    try {
      const response = await fetch(
        `/api/admin/musicians/${musician.id_m}`,
        {
          method: "PATCH",
          headers: {
