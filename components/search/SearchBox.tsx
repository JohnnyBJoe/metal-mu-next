...
const allResults = [
  ...
  function handleKeyDownInput(e: React.KeyboardEvent<HTMLInputElement>) {
    ...
    if (e.key === "ArrowDown") {
      ...
      e.preventDefault();
      setSelectedIndex((i) => (i <= 0 ? allResults.length - 1 : i - 1));
    } else if (e.key === "Enter") {
      e.preventDefault();
      if (selectedIndex >= 0) {
  ...
  selectedIndex === index ? "bg-zinc-800" : ""
          ...
          {results.persons.map((person, index) => {
            const globalIndex=results.bands.length+index;
            return (
            <Link
              ...
              onClick={closeResults}
              className={`flex items-start gap-3 px-3 py-2 transition-colors hover:bg-zinc-800 ${selectedIndex===globalIndex?"bg-zinc-800":""}`}
            >
          ...
          {results.albums.map((album, index) => {
            const globalIndex=results.bands.length+results.persons.length+index;
            return (
            <Link
              key={`album-${album.id_d}`}
              ref={(el) => (itemRefs.current[globalIndex] = el)}
              href={`/?band=${album.bandId}&album=${album.id_d}`}
              onClick={closeResults}
              className={`flex items-start gap-3 px-3 py-2 transition-colors hover:bg-zinc-800 ${selectedIndex===globalIndex?"bg-zinc-800":""}`}
            >
              <span className="mt-0.5 text-lg">💿</span>
